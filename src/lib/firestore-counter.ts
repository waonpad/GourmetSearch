...
export class Counter {
  ...
  constructor(private doc: firebase.firestore.DocumentReference, private field: string) {
    ...
    const shardsRef = firebase.firestore().collection(doc.path + '/' + SHARD_COLLECTION_ID);
  ...
  public incrementBy(val: number): Promise<void> {
    ...
    const shardRef = firebase.firestore().collection(this.doc.path + '/' + SHARD_COLLECTION_ID);
...
function getShardId(cookie: string): string {
  const result = new RegExp('(?:^|; )' + encodeURIComponent(cookie) + '=([^;]*)').exec(
    document.cookie
  );
